...
class Modal extends Nerv.Component<ModalProps, ModalState> {
  ...
  componentDidMount () {
    const { closeOnPressEsc, value } = this.props
    if (value) {
      this.setState({
        visible: true
      })
  ...
  enhanceChildren = () => {
    ...
    return Nerv.Children.map(
      ...
      (child, index) => {
        ...
        const { onCancel: oonCancel = () => {}, onConfirm: oonConfirm = () => {} } = oProps
        const { name } = child
        const newProps = {}
        switch (name) {
          case 'ModalBody':
      ...
      <CSSTransition
        ...
        <div>
          ...
          <div
            ...
            <div className={classnames('at-modal', modalClass)} style={{ width: `${width}px`, ...modalStyle }}>
              {title ? (
              ...
              {showClose ? (
                <span className='at-modal__close' onClick={this.closeBtnHandle}>
