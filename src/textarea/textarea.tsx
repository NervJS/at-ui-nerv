...
class Textarea extends Nerv.Component<TextareaProps, any> {
  ...
  render () {
    ...
    if (typeof style == 'string' || style instanceof String) {
      styleComputed = Object.assign(styleStr2Obj(style), {height: this.state.height})
    } else {
    ...
    return (
      <div className={classNames}>
        <textarea ref='textarea' value={props.value} placeholder={placeholder} autofocus={autofocus} name={props.name} disabled={disabled} resize={props.resize || 'vertical'} onInput={this.inputHandler} rows={minRows} style={styleComputed} {...needProps} className='at-textarea__original'></textarea>
      </div>
    )
  }
  inputHandler (e: MouseEvent) {
  ...
  resizeTextarea () {
    const props = this.props
    if (!props.autosize && !props.minRows && !props.maxRows) { return }
    let calculateStyle: any = null
    if (props.autosize) {
      calculateStyle = calcTextareaHeight(this.refs.textarea)
    } else {
      calculateStyle = calcTextareaHeight(this.refs.textarea, props.minRows || 2, props.maxRows)
    }
    this.setState({
