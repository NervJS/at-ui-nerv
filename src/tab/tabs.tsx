...
class Tabs extends Nerv.Component<TabsProps, any> {
  ...
  updateHandle () {
    let containerWidth = this.refs.navScroll.offsetWidth
    let navWidth = this.refs.nav.offsetWidth
    let currentOffset = this.state.navOffset
  ...
  renderIconNav (icon) {
    let classname =`icon at-tabs-nav__icon ${icon}`
  ...
  renderTabsNav () {
    let {children,closable=false} = this.props
    let closableFlag =  closable || closable === 'true'  
    let navArr:any[] = []
    let extra:any[]= []
    Nerv.Children.forEach(children as any, (child,index)=>{
      ...
      if(props.slot && props.slot=='extra') {
        ...
        let {disabled=false,label,icon} = props
        let disabledFlag = disabled || disabled === 'true'
  ...
  renderTransitionStyle () {
    //这个是tabbody的动画移动
    let animated = this.props.animated || true
  ...
  render () {
    ...
    let tabsNav = this.renderTabsNav()
    let transitionStyle = this.renderTransitionStyle ()
    let prevBtn:any
    let nextBtn:any
  ...
  renderBody () {
    let {children,animated=true} = this.props
  ...
  updateAfterUpdate (prevState,str?) {
    let flag = this.prevable || this.nextable
